...
description: "Go from an ICP description to a ranked, enriched lead list in one shot. The user describes their ideal customer via \"$ARGUMENTS\"."
---
...
- 
eveal_personal_emails` set to `true`
...
1. **Save all to Apollo** — Bulk-create contacts via `mcp__claude_ai_Apollo_MCP__apollo_contacts_create` with 
un_dedupe: true` for each lead
